...
# Capability-owned paths. Values are relative to business/.
MOVES = {
...
# Rewrite every textual reference. Longest strings first prevents partial path corruption.
replacements: dict[str, str] = {}
for old, new in MOVES.items():
    replacements[f"business/{old}"] = f"business/{new}"
    replacements[f"business\\{old}"] = f"business\\{new.replace('/', chr(92))}"
    # Top-level artifact names are canonical business-relative paths throughout validators and templates.
    if "/" not in old and "." in old:
        replacements[old] = new

# Directory-relative references that occur inside business artifacts and scripts.
for old, new in MOVES.items():
...
for path in ROOT.rglob("*"):
    if not path.is_file() or path.suffix.lower() not in text_suffixes:
        continue
    if ".git" in path.parts or "node_modules" in path.parts or path == Path(__file__):
        continue
    try:
...
# Update public architecture navigation.
...
# Release metadata and package parity.
...
# Remove temporary inventory output.
