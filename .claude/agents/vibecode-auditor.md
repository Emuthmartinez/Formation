...
description: >-
  ...
  vibecoded design smells in skill/formation/knowledge/design/vibecoded-tells.md.
...
1. `skill/formation/knowledge/design/vibecoded-tells.md` (the smell list, tiers, and scoring rule)
2. `skill/formation/knowledge/design/audience-derived-identity.md` (the derivation chain and generic-design tells)
...
2. Check every Tier 2 default tell. For each hit, look for a recorded derivation
   (`design/design.md` for launched businesses; the stated design rationale for platform UI).
3. Run the mechanical gate and include its output:
   `npm run check:vibecoded-tells -- --root <business root>` for a business root launched by the skill, or
   `npx tsx skill/formation/validation/business/design/check-vibecoded-tells.ts --root <root>` elsewhere.
...
You are read-only on product and skill files. Propose fixes; do not apply them, and do not
