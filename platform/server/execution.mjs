...
/**
 * The platform half of the platform-to-skill execution adapter (docs/platform/remaining-gaps.md,
 * "Platform-to-skill execution adapter"). It receives an authorized Formation execution request,
 ...
 * into the decision system, and imports the skill's verified results — claims, evidence,
 ...
 *   The skill's reducer and session runner stay the only writers of skill-owned state.
 ...
 * - Honest state: "the skill could not be reached" is always reported as exactly that —
...
export const defaultSkillDir = path.resolve(moduleDir, "../../skill/formation");

/**
 * Environment variables whose names look secret-bearing never cross the boundary. The skill
...
/**
 * Mirrors the skill's tooling/lib/tsx-bin.ts resolution order. That helper cannot be imported
 ...
 * of truth, stage, launch target, and founder capacity are what the skill plans against;
...
export class EngineBridge {
  ...
  /**
   * Why the skill cannot be reached, or null when it can. Public so the health probe can ask the
  ...
  #unavailableReason() {
    if (!existsSync(this.#skillDir)) return "Formation's launch automation is not installed on this server.";
    if (!this.#tsxBin) return "Formation's launch automation cannot start on this server.";
    return null;
  }
    ...
    return new Promise((resolve) => {
      ...
      const timer = setTimeout(() => {
        if (!settled) {
          settled = true;
          child.kill("SIGKILL");
          resolve({ started: false, reason: "Formation did not answer in time." });
        }
      }, timeoutMs);
      timer.unref?.();
      ...
      child.on("error", (error) => {
        if (!settled) {
          ...
          resolve({ started: false, reason: "Formation's launch automation could not be started on this server." });
        }
      });
      child.on("close", (code) => {
        if (!settled) {
  ...
  /**
   * Read-only: what does the skill's durable run for this workspace look like right now?
   * Returns { reachable: true, report } only when the skill answered with a parseable boundary
   ...
   * for one. The skill CLI it spawns opens no handle for writing, so a founder can point this at
  ...
  /**
   * Creates or resumes the durable engine run by invoking the skill's own session runner — the
   ...
   * progress; `executor` exists so tests can opt into the skill's fixture executor.
   */
  ...
  /**
   * Records a founder's answer to a parked approval through the skill's one sanctioned path,
...
export class ExecutionWorker {
  ...
  constructor(store, { engine, resolveEngineWorkspace, wallClockSeconds = 600, executor } = {}) {
    ...
    // Test-only: "fixture" opts into the skill's synthetic-completion executor. Never accepted
    // from a request — the skill's default (no real execution, no fabricated progress) is the
  ...
  async #recoverInterruptedExecutions() {
    await this.#store.transaction((database) => {
      ...
      for (const execution of database.executions) {
        ...
        // Requeueing is safe: the skill run is durable and the runner resumes it; a session the
        // dead process left behind is protected by the skill's own workspace lock.
   ...
   * only for tests that inject the skill's fixture executor — the production engine plans,
  ...
  async inspect(workspace) {
    ...
    if (!engineWorkspaceDir) {
      return { connected: false, reachable: false, reason: "This company is not connected to launch execution yet." };
    }
  ...
  async launchMatrix(workspace) {
    const engineWorkspaceDir = this.#resolveEngineWorkspace(workspace);
    if (!engineWorkspaceDir) return { available: false, reason: "This company is not connected to launch execution yet." };
    const checkedAt = new Date().toISOString();
    const view = await this.#engine.describe(engineWorkspaceDir);
    ...
    if (view.report.schemaVersion !== "1.1.0" || !view.report.launchMatrix) {
      return { available: false, checkedAt, reason: "The connected launch runtime does not support the launch matrix yet." };
    }
  ...
  /**
   * The founder approvals view for one workspace: asks the skill what it is asking, mirrors
   ...
   *
   * The same ready answer also imports the skill's verified results (claims, deliverable
   * drafts, staleness, blocker tasks): a founder checking in on the skill is the natural moment
   * for Formation's records to catch up with work the skill settled since the last session.
   *
   * `workspace.founder.operatingMode` shares the skill's grant-level vocabulary and is returned
   * for display, but it never influences this boundary: the skill's own grants and waivers
  ...
  async syncApprovals(workspace, actingUserId) {
    ...
    if (!engineWorkspaceDir) {
      ...
      return { connected: false, reachable: false, operatingMode, reason: "This company is not connected to launch execution yet.", approvals: listEngineApprovals(database, workspace.id) };
    }
    const checkedAt = new Date().toISOString();
    const view = await this.#engine.describe(engineWorkspaceDir);
    if (!view.reachable) {
   ...
   * core/session/approve.ts — the skill's only sanctioned path out of a founder gate — and the
   * decision record closes only after the skill confirmed it recorded the answer. Any failure
   * to reach the skill leaves the record open and says so: an unanswered approval stays
  ...
  async answerApproval({ workspaceId, user, decisionId, answer, reason }) {
    ...
    if (decision.status !== "open") {
      throw new ExecutionError(
        ...
        decision.status === "superseded"
          ? "The launch plan moved on before this was answered, so Formation is no longer asking it."
          : "This approval was already answered.",
      );
    }

    const engineWorkspaceDir = this.#resolveEngineWorkspace(workspace);
    if (!engineWorkspaceDir) throw new ExecutionError(409, "This company is not connected to launch execution yet.");

    // Confirm the skill still asks this exact question before recording anything: the mirror
    // could be stale (plan moved on, or answered directly with the skill's own CLI).
    const view = await this.#engine.describe(engineWorkspaceDir);
    if (!view.reachable) throw new ExecutionError(502, `Nothing was recorded: ${view.reason}`);
    if (!view.report.workspaceReady) {
      throw new ExecutionError(409, `Nothing was recorded: ${view.report.reason ?? "the launch skill has no workspace for this company yet."}`);
    }
    ...
    if ((view.report.runId ?? null) !== decision.source.runId || !live || live.status !== "pending") {
      ...
      throw new ExecutionError(409, "The launch skill is no longer asking this question. Refresh the approvals list for the current ones.");
    }
    ...
    if (!result.recorded) {
      ...
      throw new ExecutionError(502, "The launch skill could not record this answer, so nothing changed.");
    }
   ...
   * selects or accepts a stable catalog workflow against the skill's live answer, fingerprints
  ...
  async submit({ workspaceId, user, workflowId }) {
    ...
    if (!view.report.workspaceReady) {
      throw new ExecutionError(409, `Nothing was started: ${view.report.reason ?? "the launch skill has no workspace for this company yet."}`);
    }
    ...
    if (workflowId !== undefined) {
      ...
      if (!selected) {
        ...
        throw new ExecutionError(
          409,
          `Nothing is ready for the launch skill to work on right now — ${waiting} step(s) are waiting on you and ${upcoming} are waiting on earlier work.`,
        );
      }
    }
    ...
    const outcome = await this.#store.transaction((liveDatabase) => {
      ...
      liveDatabase.activity.push({
        ...
        title: `Formation asked to work on ${chosenTitle}`,
        detail: "Formation sent this step to the launch skill with the company's current context.",
        actor: user.name,
        createdAt: now,
      });
  ...
  async kick() {
    ...
    try {
      while (await this.#processNext()) {
        // Drain the durable queue in order; one session at a time keeps the skill's own
  ...
  async #processNext() {
    ...
    try {
      ...
      const engineWorkspaceDir = this.#resolveEngineWorkspace(workspace);
      if (!engineWorkspaceDir) throw new Error("This company is not connected to launch execution.");
      ...
      await this.#store.transaction((nextDatabase) => {
        ...
        if (!session.ran) {
          ...
          live.error = `The launch skill could not be started for this work. ${session.reason}`;
          ...
          if (after?.reachable && after.report.workspaceReady) {
            ...
            if (live.engine?.runId && runId && live.engine.runId !== runId) {
              live.notes.push("The launch skill started a fresh run because its plan changed since this was requested.");
            }
            ...
            live.notes.push("The work session finished, but the skill's latest state could not be read afterwards.");
          }
        } else if (session.code === 2) {
          live.status = "failed";
          live.failureKind = "engine_busy";
          live.error = "The launch skill was already busy with another session for this company. Nothing was lost — ask again in a minute.";
        } else if (session.code === 3) {
          live.status = "failed";
          live.failureKind = "engine_integrity";
          live.error = "The launch skill stopped because this company's saved files changed outside its normal process. It needs a careful look before more work runs.";
        } else {
          live.status = "failed";
          live.failureKind = "session_error";
          live.error = "The launch skill session did not complete. Formation kept its own record of what happened.";
        }
          ...
          detail:
            live.status === "completed"
              ? live.importedResults && live.importedResults.verifiedResults === 0
                ...
                : "The skill session finished and the run state was refreshed."
              : (live.error ?? "The skill session did not complete."),
          actor: "Formation",
      ...
      await this.#store.transaction((database) => {
        ...
        live.status = "failed";
        live.failureKind = "engine_unreachable";
        live.error = "The launch skill could not be reached for this work, so it did not run.";
 ...
 * ids, domain ids, and evaluator vocabulary stay on the skill side of the boundary.
 */
export function founderRunView(report) {
  ...
  if (counts.held > 0) parts.push(`${counts.held} waiting for a go-ahead the skill does not have`);
