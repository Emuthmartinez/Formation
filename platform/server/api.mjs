...
export async function handleApi({ request, response, url, store, worker, allowDemoAuth, allowRegistration, authLimiters }) {
  assertSameOrigin(request);
  const method = request.method ?? "GET";
  const pathname = url.pathname;
  const context = { request, response, method, pathname, store, worker, allowDemoAuth, allowRegistration, authLimiters };
