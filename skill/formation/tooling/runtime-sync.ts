 ...
 * installed skill runtime (default: ~/.codex/skills/formation).
 *
 ...
 *   --installed <dir>   Installed runtime root (default: ~/.codex/skills/formation).
...
function parseOptions(argv: string[]): Options {
  ...
  const options: Options = {
    ...
    installedRoot: path.join(homedir(), ".codex", "skills", "formation"),
    force: false,
    adopt: false,
    verify: true,
    json: false,
...
/** Alias written by installs that predate the formation rename; reported so a stale one is visible, never repaired here. */
const LEGACY_ALIAS_NAME = "b2c-mobile-business-launch";

function verifySymlinkAliases(installedRoot: string): void {
  const aliasRoots = [path.join(homedir(), ".claude", "skills"), path.join(homedir(), ".agents", "skills"), path.join(homedir(), ".cursor", "skills")];
  for (const aliasRoot of aliasRoots) {
    if (!existsSync(aliasRoot)) continue;
    for (const aliasName of [path.basename(installedRoot), LEGACY_ALIAS_NAME]) {
      const alias = path.join(aliasRoot, aliasName);
      try {
        ...
        if (aliasName !== LEGACY_ALIAS_NAME) {
