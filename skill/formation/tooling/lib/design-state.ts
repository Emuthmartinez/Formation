...
function validateCrossSurfaceState(state: unknown, tokens: unknown, filePath: string): Issue[] {
  ...
  if (!isRecord(latestVersion)) {
    ...
    const renderedArtifacts = asArray(latestVersion.renderedArtifacts)
      .map((entry) => asString(entry))
      .filter(Boolean);
    if (!statePaths.includes("studio/seed/business.json")) {
      issues.push(issue("error", "design_state.version_log.business_missing", "Latest version entry must include studio/seed/business.json.", filePath));
    }
    if (!statePaths.includes("studio/seed/theme.tokens.json")) {
      issues.push(issue("error", "design_state.version_log.tokens_missing", "Latest version entry must include studio/seed/theme.tokens.json.", filePath));
    }
