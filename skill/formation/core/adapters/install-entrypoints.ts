...
const MANAGED_MARKER = "formation";
/** Marker written by installs that predate the formation rename; still matched so their entries strip cleanly. */
const LEGACY_MANAGED_MARKER = "b2c-mobile-business-launch";
...
function isManagedEntry(entry: HookEntry): boolean {
  if (entry?._managed === MANAGED_MARKER || entry?._managed === LEGACY_MANAGED_MARKER) {
    return true;
  }
  // Command-text fallback stays legacy-only: v1 installs never stamped _managed, but always named the
  // skill in the command. The new marker is matched by equality alone — "formation" is too short a
  // substring to scan founder-authored commands for without false positives.
  return (entry?.hooks ?? []).some((hook) => typeof hook?.command === "string" && hook.command.includes(LEGACY_MANAGED_MARKER));
}
...
export interface InstalledRuntimeManifest {
  ...
  skillVersion: string;
  skillRoot: string;
  skillRootEnv: "FORMATION_SKILL_ROOT";
  installedSkillName: "formation";
