...
`formation` autonomous runtime (v2). This file is the canonical entrypoint for
...
Resolve the skill root from `FORMATION_SKILL_ROOT` first, then the configured path, then the
