...
export function register(h: Harness): void {
  ...
  runFixture("shipped Deletion Manifest template passes base mode: its unresolved option-list row is not a deletion claim", cutoverBaseline, cutoverScript, 0);

  const cutoverUnverifiable = makeFixture("onboarding-cutover-deletion-unverifiable");
  mutateOnboarding(cutoverUnverifiable, (text) =>
    text.replace(
      "| `LEGACY-001` | Record the item | Preserve durable data, transform once, recompute, archive legal history, or delete |",
      "| LEGACY-001 | Record the item | delete |",
    ),
  );
  runFixture(
  ...
  mutateOnboarding(cutoverNotVerified, (text) =>
    text.replace(
      "| `LEGACY-001` | Record the item | Preserve durable data, transform once, recompute, archive legal history, or delete |",
      "| `state/PROJECT_STATE.yaml` | Record the item | delete |",
    ),
  );
  runFixture(
  ...
  mutateOnboarding(cutoverVerified, (text) =>
    text.replace(
      "| `LEGACY-001` | Record the item | Preserve durable data, transform once, recompute, archive legal history, or delete |",
      "| `legacy/old-onboarding-flow.ts` | Record the item | delete |",
    ),
  );
  ...
  // decision, and its placeholder span (`LEGACY-001`, absent from disk by construction) then
  ...
  mutateOnboarding(cutoverOptionListNotAClaim, (text) => text.replace("| `LEGACY-001` | Record the item |", "| `state` | Record the item |"));
  runFixture(
