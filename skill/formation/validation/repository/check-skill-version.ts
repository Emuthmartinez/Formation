  ...
  issues.push(
    issue(
      ...
      [
        ...
        "or re-run with --source /path/to/checkout/skill/formation or --remote-url.",
      ].join(" "),
    ...
    if (comparison === undefined) {
      ...
      issues.push(
        issue(
          ...
          "skill_version.stale",
          [
            `Installed Formation skill is ${installed.manifest.version}; latest available source is ${latest.manifest.version}.`,
            ...
            "If the founder approves, sync the source skill into ~/.codex/skills/formation, run npm install, run npm run audit, and verify the Claude/Agents/Cursor symlinks and legacy aliases.",
          ].join(" "),
          path.relative(process.cwd(), path.join(args.installed, "skill-version.json")),
        ),
      );
...
function parseArgs(argv: string[]): VersionArgs {
  ...
  const source = explicitSource ?? (process.env.FORMATION_SKILL_SOURCE ? path.resolve(process.env.FORMATION_SKILL_SOURCE) : findDefaultSource());
  let installed = flagString(flags, "installed") ?? (process.env.FORMATION_SKILL_INSTALLED ? path.resolve(process.env.FORMATION_SKILL_INSTALLED) : skillRoot);
  ...
  return {
    ...
    sourceExplicit: Boolean(process.env.FORMATION_SKILL_SOURCE) || explicitSource !== undefined,
  };
}

function findDefaultSource(): string {
  const candidates = [process.env.FORMATION_SKILL_SOURCE, skillRoot].filter((candidate): candidate is string => Boolean(candidate));
 ...
 * agree: the manifest's own sourcePath ("skill/formation") is the tail of the
...
https.get(url, { headers: { "user-agent": "formation" } }, (response) => {
