...
```text
A newer Formation skill is available: installed <installed_version>, latest <latest_version>.
...
```bash
npm run check:skill-version -- --source skill/formation --installed ~/.codex/skills/formation
npm run check:version-discipline -- --repo-root . --skill-root skill/formation
```
...
npm run check:skill-version -- \
  ...
  --remote-url https://raw.githubusercontent.com/Emuthmartinez/Formation/main/skill/formation/skill-version.json
```
...
npm run check:skill-version -- --source "$HOME/code/Formation/skill/formation" --installed .
```
