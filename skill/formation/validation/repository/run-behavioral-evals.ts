 ...
 * GitHub Actions workflow or locally. See validation/repository/launchbench-evals.md
 ...
 * Usage:
 *   tsx tooling/run-behavioral-evals.ts --list
 *   ANTHROPIC_API_KEY=... tsx tooling/run-behavioral-evals.ts [--only id1,id2]
...
import { asArray, asString, flagBoolean, flagString, isRecord, parseFlags } from "../../tooling/lib/launch-state.js";
...
async function run(toRun: BehavioralScenario[]): Promise<void> {
  ...
  const systemPrompt = [
    "You are an autonomous launch agent with the Formation skill loaded.",
...
function collectScenarios(): BehavioralScenario[] {
  ...
  for (const source of ["validation/repository/evals/launchbench", "validation/repository/evals/agent-behavior"]) {
...
function parseArgs(argv: string[]): Args {
  ...
  return {
    ...
    out:
      ...
      path.join(skillRoot, "validation", "repository", "evals", "behavioral-results", `behavioral-${new Date().toISOString().replace(/[:.]/g, "-")}.json`),
