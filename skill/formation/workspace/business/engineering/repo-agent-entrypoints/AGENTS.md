...
If a listed file does not exist yet, create or update it through the relevant `formation` reference instead of inventing a one-off replacement.
...
- Use `formation` as the default workflow for broad launch/business work, business-side setup, App Store or Google Play readiness, RevenueCat/Stripe/PostHog/Resend setup, MobAI/native iOS proof, security release work, GEO/SEO, UGC/Fastlane, and production-readiness claims.
...
- Never dispatch a role prompt alone. Use the graph's assembled brief: `AGENTS.md` → `APP_AGENTS.md` → role prompt → mandatory task references → matching role context packs → matching installed skills → current tool routes. Require the five-part knowledge receipt before accepting worker output.
