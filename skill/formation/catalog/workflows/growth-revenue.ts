...
export const workflows = [
  ...
  workflow({
    ...
    instructions:
      "Build a runnable local landing site in growth/landing/ as soon as design/design.md is accepted. Before this node enters the dispatch frontier, the orchestration manager must invalidate workflow.design.design-room-state-mutate-version-render in run state (mark it stale, clear its blocker and accepted output fingerprint, and mark its output bindings unaccepted), recompute the frontier, redispatch that Design Room node with a landing-specific change classification and affected scope, accept its four refreshed outputs, and only then recompute and dispatch this landing node. The landing worker never writes Design Room outputs. Run check:design-room before changing the landing hero, scroll behavior, micro-interactions, or any other visual surface; an accepted Design Room dependency from an earlier or unrelated scope is not evidence for a later landing mutation. Do this work in parallel with app implementation. Use the launch-surface-producer prompt in agents/launch-surface-producer.md. Include one conversion goal, the approved promise, a mobile CTA, proof, responsive behavior, and a local build check. Select optional sections from audience, story, proof, and conversion evidence; do not apply a default section recipe. Include a short web onboarding or qualification flow only when the product contract needs it. Use current COPY_DECK.md strings. Show prices only when revenue/REVENUE_OPS.md records founder-approved prices. Create truthful screenshot slots and an update route, but label design renders as previews until real app captures exist. Declare landing_viewed, landing_cta_clicked, and waitlist_submitted before implementation. Write growth/landing/surface-contract.json with SHA-256 digests for every canonical input, each Tier 1 locale, pricing/onboarding applicability decisions, screenshot evidence, and local build proof. Add a scrollytelling object with applicable, evidence, locales, scenes, and qa. When scrollytelling applies, record ordered situation-to-mechanism-to-outcome-to-proof scenes. Each scene records narrative_roles, states, visual_job, source_kind, asset_id for heavy media and poster_asset_id only when save_data is poster, evidence_id, localizations, activation_guides, modes, and forward_reverse. Each localization records locale, beats with state_id, exact text, copy_key, and copy_sha256, plus caption and accessible_description objects with text, copy_key, and copy_sha256. Each QA row records browser, platform, viewport, locale, direction, mode, scene_id, expected_state, result, and evidence. Record current Chrome, Safari, and Firefox proof for short and tall desktop viewports. Record iOS Safari and Android Chrome proof for short and tall mobile viewports. Pass the request-derived saveData prop during SSR. When it is true, render the approved poster or omit the heavy source; CSS hiding does not stop an image request. When scrollytelling does not apply, record the evidence-based reason and empty scenes and qa lists. Keep motion progressive per landing-motion-craft.md and editorial-scrollytelling.md. When the 60fps.design MCP is connected, ground hero, scroll, and micro-interaction choices in live exemplars (60fps_search_shots and 60fps_get_motion_breakdown). Re-express each mechanism on the web --motion-* scale. Never port SwiftUI starter code to a web surface. Run check:design-room, check:vibecoded-tells, and check:scrollytelling before handoff. Do not deploy from this node.",
    reads: [
      "GEO_SEO.md",
