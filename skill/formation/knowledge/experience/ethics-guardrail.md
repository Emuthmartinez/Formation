...
- `knowledge/design/design-room.md` / `knowledge/design/design-visual-system.md` — motion fallbacks required
...
All codes are emitted by `validation/business/experience/check-emotional-design.ts` with the prefix `emotional_design.` (audit-artifact codes use `emotional_audit.`).
