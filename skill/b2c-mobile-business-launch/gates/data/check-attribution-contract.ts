...
import { asArray, asBoolean, asString, collectFiles, getPath, issue, loadProjectState, parseCliArgs, reportAndExit } from "../../scripts/lib/launch-state.js";
 ...
 * We locate the example by walking up from __dirname (gates/data/) to the skill
...
function isExampleIdentical(filePath: string): boolean {
  try {
    ...
    // gates/<domain>/ — so the skill root, and business/ with it, is two levels up.
