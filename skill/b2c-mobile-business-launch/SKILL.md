...
| Copy + app strings | before writing any words a user reads: conversion copy (hero/CTA, store, paywall) and every in-app string (buttons, empty states, errors, primes, settings); and l10n readiness | [`references/conversion-copy.md`](references/conversion-copy.md) | `COPY_DECK.md` holds every string, `COPY_BRIEF.md` the voice; builders consume, never invent · `check:app-copy`, `check:no-slop` |
