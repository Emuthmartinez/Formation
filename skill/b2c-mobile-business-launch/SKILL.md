...
4. This file *routes*. Phase detail lives in [`playbook/process/launch-phases.md`](playbook/process/launch-phases.md); artifact acceptance criteria live in [`playbook/process/artifact-contracts.md`](playbook/process/artifact-contracts.md); completeness auditing lives in [`playbook/process/launch-coverage.md`](playbook/process/launch-coverage.md).

## Always-On Contracts

Never speak this file's internal vocabulary to a founder: no lane ids, no phase codes, no status values like `not_started` or `blocked`, no bare "gate," and no tool names such as AskUserQuestion. Say what is actually happening, in plain language. Load [`playbook/words/no-slop-writing.md`](playbook/words/no-slop-writing.md) before writing anything a founder reads.
...
| --- | --- | --- |
| Running the launch | starting, resuming, or auditing a launch: phases, durable state, coverage, artifact contracts, traceability, subagent orchestration, dynamic workflows, or propagating a change across surfaces | [`playbook/process/README.md`](playbook/process/README.md) |
...
The full per-lane evidence checklist is the Coverage Matrix and Handoff Completeness Checklist in [`playbook/process/launch-coverage.md`](playbook/process/launch-coverage.md); per-artifact acceptance criteria are in [`playbook/process/artifact-contracts.md`](playbook/process/artifact-contracts.md). Audit against those before claiming readiness, and let no known miss hide behind prose.
