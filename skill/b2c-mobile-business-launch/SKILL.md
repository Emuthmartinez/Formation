...
4. This file *routes*; phase detail, artifact acceptance criteria, and completeness auditing live in `knowledge/process/` and are linked where they apply.
## Always-On Contracts
...
3. **Compute the ready frontier.** `npm run plan:frontier` compiles the catalog and reports what is ready now, what is parked on a founder decision, and which ready work is parallel-safe. Order lane work from that, not by reading this file top to bottom, and re-run it after any state or grant change.
