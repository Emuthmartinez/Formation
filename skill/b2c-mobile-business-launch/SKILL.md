...
| Change cascade | after **any** change to a launched or near-launch app's features, copy, brand vocabulary, pricing, products, design, or data behavior | [`references/change-cascade.md`](references/change-cascade.md) | in-app, listing/keywords/screenshots/App Preview/IAP/localizations, RevenueCat, landing + meta/JSON-LD, SEO/GEO, email, analytics, and legal all propagated |
...
| Tool recipes | running research, setting up the funnel, configuring domain/email routing, checking analytics, choosing stack, or verifying deployment | [`references/tool-recipes.md`](references/tool-recipes.md) | any fast-moving CLI path records its current documentation basis instead of relying on stored examples |
