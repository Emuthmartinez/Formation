 ...
 *
 * docs/architecture.md splits the skill's executables three ways: `validation/business/` proves a
 * business launch, `validation/repository/` proves the skill itself, and `tooling/` keeps the
 ...
 * `tooling/lib` and `validation/repository/fixtures` are deliberately excluded: they are
 * imported, never spawned, and indexing them would let a module basename mask
 * a real collision between two runnable scripts.
 */
export const SCRIPT_ROOTS = ["validation/business", "validation/repository", "tooling"] as const;
