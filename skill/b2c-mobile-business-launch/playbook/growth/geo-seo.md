...
- for localized landing surfaces, `hreflang` tags, localized `llms.txt`/schema, and translated answer sections **only for the markets ranked Tier 1 in `LOCALIZATION_MARKET_RESEARCH.md`** (see [`localization-market-research.md`](../research/localization-market-research.md)) — localize web demand the same research-first way as the store, not by translating every page into every language
...
- before drafting title tags, meta descriptions, or answer-style sections, load `playbook/words/no-slop-writing.md`; weasel attribution and importance puffery here are also a copy-compliance risk (see §4 below), not just style noise
...
Run this scan before writing or accepting any landing copy, not only before deploy. These are trust-breaking patterns that have required founder corrections after deployment. Run `playbook/words/no-slop-writing.md`'s self-check (§6) and `check:no-slop` alongside this scan — weasel attribution and importance puffery are both style slop and a compliance risk here.
...
This check is required before drafting privacy policy, terms of service, support pages, or any public page where a contact address appears. See also `playbook/trust/privacy-terms.md` section 5: "contact emails listed on the pages receive mail and have an owner."
