...
| Post-launch loop | `POST_LAUNCH_OPS.md` weekly operating rhythm (crash triage, review responses with SLA, release/hotfix cadence, retention review, support ops) with a dated weekly log carrying real numbers, `lanes.post_launch_ops.live_since` anchoring the clock, `LAUNCH_RETRO.md` checkpoints completed on time with its day-30/day-90 Kill, Hold, Or Scale verdict, weekly ASO/reviews/rankings/conversion/retention/revenue/competitor deltas, `PORTFOLIO_REGISTRY.md` once a second business exists | `post-launch-operations.md`, `control-plane.md` portfolio layer, AppKittie, ASO skills, RevenueCat, PostHog, Sentry, `check:post-launch`, `check:portfolio-registry` |
...
- Higgsfield-generated landing-page hero and feature-section visuals (product-photoshoot `hero_banner` / `lifestyle_scene` / `ad_creative_pack` modes, DESIGN.md brief required); route recorded in `CONTENT_ASSETS.md` — see the **Cheap-First Direction (z_image → production model)** recipe in `tool-recipes/visual-and-motion-production.md`
- Higgsfield-generated press/PR imagery (product-photoshoot `lifestyle_scene` or `conceptual_product` mode, or `gpt_image_2` for flat editorial art, DESIGN.md brief required); route recorded in `CONTENT_ASSETS.md` — see the **Cheap-First Direction (z_image → production model)** recipe in `tool-recipes/visual-and-motion-production.md`
