...
Each shell must live in `state/business.json` under `controlPlane.panels`, include `stateRefs`, include `renderedArtifacts`, and appear in `design/design-room.html`. Future panel depth should read the same state store and theme tokens rather than inventing parallel docs.
...
- Let `state/PROJECT_STATE.yaml` remain the launch lane/status cockpit while `state/business.json` grows into the cross-surface business model.
