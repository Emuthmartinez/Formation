...
export const workflows = [
  ...
  workflow({
    ...
    instructions:
      "At the start of every broad launch, run the Business Bootstrap Sequence in order — confirm business identity and founder-owned contact, establish the ownership spine (recovery email/phone, 2FA), install/verify Doppler as the default secret manager, inventory existing accounts, then create/claim/delegate only what's authorized — and keep exactly one structured `activeFounderGate` in `operations/business-access.json` at a time, set to `null` while the agent has enough state to keep working. Never dump the full launch checklist on the founder: present one plain-language next decision via AskUserQuestion with the recommended choice first, defined jargon, and an explicit skip/defer route, then continue immediately into the next agent action yourself. Doppler login and social-platform login stay founder-only actions — never request a password, 2FA code, recovery code, or session token in chat — everything else (research, drafts, setup, verification) is the agent's job. `check:founder-operator` fails when access setup completes but the next business operation never starts, or when a deferred gate is treated as silent consent instead of being re-presented after 30 days.",
  ...
  workflow({
    ...
    instructions:
      "Before any authenticated action on a provider, social platform, or store console, classify it into one of seven action classes (observe/draft/mutate/publish/spend/release/destructive) and confirm it binds to a matching approval envelope in `operations/agent-operations.json` — envelopes need an exact account/project/environment, allowed classes, and expiry, and are consumed one-shot or standing, never widened by analogy. Select the narrowest reliable route (purpose-built connector, then API/CLI, then authenticated browser, manual packet last), capture sanitized before-state, act only inside the envelope, then read back the result from the provider itself rather than trusting a click or exit code. Sticky identity/legal fields — app name, bundle ID, seller identity, banking/tax, pricing, credential roles — stay founder-gated even inside an otherwise-approved mutate envelope. Reconcile every completed action into `operations/AGENT_OPERATIONS.md`, cross-reference `operations/PROVIDER_PROOF.md` for provider-backed lanes, and run `npm run check:agent-operations` before claiming any authenticated operation, provider mutation, or public response is ready.",
