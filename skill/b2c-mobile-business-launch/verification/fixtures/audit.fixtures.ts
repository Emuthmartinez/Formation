...
function buildFaithfulParityFixture(harness: Harness): { repoRoot: string; skillRoot: string } {
  ...
  const tempSkillRoot = path.join(tempRepoRoot, "skill", "b2c-mobile-business-launch");
  for (const [from, toRelativeToRepoRoot] of [
    ...
    [path.join(skillRoot, "package.json"), "skill/b2c-mobile-business-launch/package.json"],
    [path.join(skillRoot, "package-lock.json"), "skill/b2c-mobile-business-launch/package-lock.json"],
    [path.join(skillRoot, "skill-version.json"), "skill/b2c-mobile-business-launch/skill-version.json"],
  ] as const) {
