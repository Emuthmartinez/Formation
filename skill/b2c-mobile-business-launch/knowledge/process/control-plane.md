...
`design/design-room.html` and `dist/design-room/` render the Design Room from:
...
- `tooling/render-business-control-plane-workspace.ts`
...
Each shell must live in `studio/seed/business.json` under `controlPlane.panels`, include `stateRefs`, include `renderedArtifacts`, and appear in `design/design-room.html`. Future panel depth should read the same state store and theme tokens rather than inventing parallel docs.
...
- Let `state/PROJECT_STATE.yaml` remain the launch lane/status cockpit while `studio/seed/business.json` grows into the cross-surface business model.
...
- `business/strategy/PORTFOLIO_REGISTRY.md` is the artifact: one row per business (stage, MRR trend, latest Kill, Hold, Or Scale verdict, founder hours), an allocation paragraph, a cross-app learnings table fed by each `operations/LAUNCH_RETRO.md`, and the next-launch pipeline. It lives in the founder's own workspace, never inside a single app's repo, and refreshes at each app's day-30/day-90 retro checkpoints.
