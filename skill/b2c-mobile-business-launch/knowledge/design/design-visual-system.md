...
5. **Taste for direction.** Derive the identity from the audience first: [`audience-derived-identity.md`](./audience-derived-identity.md) turns `strategy/RESEARCH.md` facts into palette/type/motion/imagery decisions and names the generic-template tells — `design/design.md`'s Audience And Identity section records the result and `check:design-room` requires it. Then apply this skill's own [`quality-lens.md`](./quality-lens.md) Anti-Generic Checks — it is the in-repo, concrete taste contract with a Review Prompt built for exactly this step. Then use Taste-style review (Source Tooling above) to avoid generic output, set variance/motion/density, and choose a design language that fits the category instead of default SaaS patterns.
