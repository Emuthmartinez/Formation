...
- `SCREENSHOT_RUBRIC.md`: the grading rubric and separate-pass grader protocol behind `grade-screenshots.ts` and `check:store-screenshots`. Seed it from [`business/store/app-store-listing/SCREENSHOT_RUBRIC.md`](../../workspace/business/store/app-store-listing/SCREENSHOT_RUBRIC.md) next to `SCREENSHOTS.md` so the grader agent scores against the same dimensions the validator enforces.
...
- `APP_STORE_LISTING.md` exists as a file artifact (not just inline chat copy) and `npm run check:store-console -- --root .` passes
