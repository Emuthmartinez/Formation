...
**Script bank and judge panel.** A script earns filming or generation spend by surviving a judge panel, not by reading well to its author. Run the panel as separate reviewer passes with fresh context (subagents, or the council mechanism), each judge with exactly one job — pacing, vocabulary, idea strength, structure. Ground at least one judge entirely in a real creator's transcript corpus, and pick an imperfect source on purpose: polished writing is not what a UGC script should sound like, and a panel with no real-creator judge drifts every script back toward brand copy. The loop is write → panel tears it apart → rewrite → run it again. A script that survives is worth spending money on; one that does not is free to kill. Write 10+ scripts per batch — the panel is cheap next to one generated clip, and discovery needs volume. Record survivors in `ugc/script-bank.md` with format id and judge notes. For synthetic UGC the same gate is hard: Recipe 7 in `tool-recipes/visual-and-motion-production.md` starts at this panel, never at the model.
...
**Synthetic founder-voice content (before real creators are onboarded).** Marketing Studio supports four UGC-family modes suited to pre-roster founder-led content: `ugc`, `ugc_how_to`, `ugc_unboxing`, and `product_review`. Use these to generate synthetic founder-voice ads during format discovery before a real creator roster is running. Synthetic UGC complements real creator content — it does not replace it; real creators produce authentic human variability, comment-bait tension, and niche credibility that generated video cannot replicate. See the **Soul-Once Founder-Face Ads** recipe in `tool-recipes/visual-and-motion-production.md`; when no founder face should appear, use the **Script-First Believable-Person UGC** recipe (Recipe 7) in the same file — a first-frame image plus a Seedance 2.5 reference-driven video, no Soul or avatar required.

Guardrails for synthetic UGC:
- Script first. Every synthetic UGC generation uses a script that survived the script-bank judge panel above. The model renders a weak script as beautifully as a strong one — generation never rescues writing.
- Prompt register. UGC-family modes and Recipe 7 follow the **UGC Realism Prompt Structure** in `tool-recipes/visual-and-motion-production.md` — social context truth, imperfect presence, no resolution — never the premium product-ad structure. Polish is the tell that kills both the ad read and the AI read.
- Believability. Before scaling a synthetic format, one real human who did not make the clip watches it. Clocked as AI or as an ad → the script is the suspect first, then the prompt, last the model.
- Automation last. Scale the flow into an automated pipeline only after it passes the believability test repeatedly by hand. Automating a broken flow just makes slop faster.
