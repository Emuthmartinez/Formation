...
- Believability. Before scaling a synthetic format, one real human who did not make the clip watches it. If they identify it as AI or as an ad, suspect the script first, then the prompt, last the model. Record the result in the manifest `believability` field; a done-tier status without it fails `check:content-assets`.
