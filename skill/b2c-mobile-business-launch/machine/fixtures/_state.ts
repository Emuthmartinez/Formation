...
export function readState(root: string): MutableRecord {
  return expectRecord(parseYaml(readFileSync(path.join(root, "state/PROJECT_STATE.yaml"), "utf8")), "state/PROJECT_STATE.yaml");
}

export function writeState(root: string, state: MutableRecord): void {
  writeFileSync(path.join(root, "state/PROJECT_STATE.yaml"), stringifyYaml(state), "utf8");
}
...
export function getLane(state: MutableRecord, name: string): MutableRecord {
  ...
  return expectRecord(lanes[name], `state/PROJECT_STATE.yaml lanes.${name}`);
}

export function getTools(state: MutableRecord): MutableRecord {
  return expectRecord(state.tools, "state/PROJECT_STATE.yaml tools");
}
