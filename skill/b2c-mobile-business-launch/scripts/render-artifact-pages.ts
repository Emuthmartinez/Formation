 ...
 * Usage: tsx scripts/render-artifact-pages.ts [--root /path/to/business] [--check]
 */
    ...
    if (!existsSync(markdownPath)) {
      issues.push(
        issue(
          ...
          `${html} is rendered from ${entry.markdown}, which is missing. Restore the source document or change its entry in scripts/lib/artifact-pages.ts.`,
          entry.markdown,
        ),
      );
      continue;
