...
| Perceived Effort Delay | MEDIUM | Fake labor / sleep-timer illusion; deceptive spinner | ≥50% steps map to real computation; delay capped | `bright_line`, `dark_line`, `guardrail`, `posthog_event`, `effort_truthfulness_attestation`, `computation_type` (∈ real_api_call \| real_data_processing \| ui_composition), `max_delay_ms` |
...
| `risk_tier_conflict` / `risk_tier_duplicate_row` / `risk_tier_mismatch` / `risk_tier_unrecognized` / `risk_table_unparsed` / `card_index_unparsed` / `risk_table_malformed_row` | error | The §3 risk table carries two rows for one mechanism (disagreeing tiers, agreeing duplicates, or a bucket/explicit contradiction — one tier per mechanism), the `experience-cards.md` index Risk column disagrees with the §3 tier for a card, a tier cell is not LOW/MEDIUM/HIGH or an ordered range (ranges span their intermediate tiers), a table-shaped line fails to parse as a row, or either table stops parsing. |
