...
| `risk_tier_conflict` / `risk_tier_duplicate_row` / `risk_tier_mismatch` / `risk_tier_unrecognized` / `risk_tier_unmapped_card` / `risk_table_unparsed` / `card_index_unparsed` / `risk_table_malformed_row` | error | The §3 risk table carries two rows for one mechanism (disagreeing tiers, agreeing duplicates, or a bucket/explicit contradiction — one tier per mechanism), the `experience-cards.md` index Risk column disagrees with the §3 tier for a card, an index card resolves to no §3 row at all (deliberately unmapped cards excepted), a tier cell is not LOW/MEDIUM/HIGH or an ordered range (ranges span their intermediate tiers), a table-shaped line fails to parse as a row, or either table stops parsing. |
