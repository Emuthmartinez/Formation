...
| `risk_tier_conflict` / `risk_tier_duplicate_row` / `risk_tier_mismatch` / `risk_tier_unrecognized` / `risk_table_unparsed` / `card_index_unparsed` | error | The §3 risk table carries two explicit rows for one mechanism (disagreeing tiers or agreeing duplicates — one row per mechanism), the `experience-cards.md` index Risk column disagrees with the §3 tier for a card, a tier cell is not LOW/MEDIUM/HIGH or a range, or either table stops parsing. |
