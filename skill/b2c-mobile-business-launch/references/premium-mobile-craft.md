...
- **SwiftUI:** `matchedGeometryEffect(id:in:)` with one `@Namespace` shared between source and destination; on iOS 18+ `navigationTransition(.zoom(sourceID:in:))` covers the grid→detail case natively. The traveling element animates with `PremiumMotion.standard`; an earned object arriving may land with the celebrate family (`motion-craft-benchmarks.md` R3).
