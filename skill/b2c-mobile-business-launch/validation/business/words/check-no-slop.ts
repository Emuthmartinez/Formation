 ...
 * The rule lists live in knowledge/words/no-slop-writing.md and are parsed from there, so the
 ...
 * Usage: tsx validation/business/words/check-no-slop.ts --skill-root /path/to/skill [--root /path/to/templates] [--repo-root /path/to/repo]
 */
...
const templatesRoot = path.resolve(flagString(flags, "root") ?? path.join(skillRoot, "workspace", "business"));
const repoRootFlag = flagString(flags, "repoRoot");
const repoRoot = repoRootFlag ? path.resolve(repoRootFlag) : undefined;
const issues: Issue[] = [];

const referenceRelative = "knowledge/words/no-slop-writing.md";
...
/**
 * This repo's own public docs. README.md tells readers that knowledge/words/no-slop-writing.md
...
function listReferences(): string[] {
  // Both knowledge roots, not just knowledge/. The maintainer guidance documents
  // moved to validation/repository/ when references/ was split; scanning only knowledge/ would
