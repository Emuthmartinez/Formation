 ...
 *
 * workspace/business/engineering/repo-agent-entrypoints/settings.json ships PostToolUse hooks that
 * fire depth-check validators after artifact writes and block the screenshot
 * grading pass. validation/repository/fixtures/hooks.fixtures.ts proves those commands
 ...
 * Usage: tsx validation/business/process/check-hooks-installed.ts --skill-root /path/to/skill [--root /path/to/business-repo]
 */
import { existsSync, readFileSync } from "node:fs";
import { spawnSync } from "node:child_process";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { expandHome, flagString, issue, parseFlags, reportAndExit, type Issue } from "../../../tooling/lib/launch-state.js";
import {
  ...
  type HookEntry,
  type HookSettings,
} from "../../../tooling/lib/hook-contract.js";
