...
export function register(h: Harness): void {
  ...
  runFixture("artifact template coverage passes", path.join(skillRoot, "workspace", "business"), "check-artifact-templates.ts", 0, undefined, [
    "--skill-root",
    skillRoot,
  ]);
  ...
  runFixture("agent behavior eval definitions pass", path.join(skillRoot, "validation/repository/evals/agent-behavior"), "run-agent-evals.ts", 0);
  ...
  runFixture("template secret docs pass from bundled template path", path.join(skillRoot, "workspace", "business"), "check-secret-routing.ts", 0);
  ...
  writeFileSync(
    ...
    readFileSync(path.join(skillRoot, "workspace", "business", "state/PROJECT_STATE.yaml"), "utf8"),
    "utf8",
  );
  ...
  const withLandingSite = (root: string, html: string): void => {
    ...
    writeFileSync(
      path.join(root, "state/PROJECT_STATE.yaml"),
      readFileSync(path.join(skillRoot, "workspace", "business", "state/PROJECT_STATE.yaml"), "utf8"),
      "utf8",
    );
