...
1. Run `npx tsx scripts/grade-screenshots.ts --root . --state state/PROJECT_STATE.yaml` to generate the grading task template. This reads SCREENSHOTS.md, verifies each final PNG exists, and reads its real IHDR width/height.
