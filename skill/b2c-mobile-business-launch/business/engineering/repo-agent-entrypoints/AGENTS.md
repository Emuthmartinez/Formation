...
- Design Room: `state/business.json`, `state/theme.tokens.json`, `design/design-room.html`
...
- Design Room state: `state/business.json`, `state/theme.tokens.json`, `design/design-room.html`, and `dist/design-room/`
...
- Use `playbook/engineering/engineering-orchestration.md`, `playbook/orchestration/parallel-agent-orchestration.md`, and `playbook/engineering/app-agent-roster.md` from the skill before editing `AGENTS.md`, `CLAUDE.md`, `APP_AGENTS.md`, `operations/ORCHESTRATION.md`, `engineering/ENGINEERING_PLAN.md`, or `engineering/PRODUCTION_READINESS.md`.
...
- All design work follows STATE -> MUTATE -> VERSION -> RENDER. Mutate `state/business.json` and `state/theme.tokens.json`, render `design/design-room.html`, and version/baseline with git instead of creating one-off design proposal files.
