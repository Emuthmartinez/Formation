...
`Hero.tsx` exposes a `videoSrc`/`poster` slot for a Remotion-rendered loop (the Mux-style hero). It is **opt-in per launch** and license-gated: route through [`playbook/design/remotion-content-assets.md`](../../../playbook/design/remotion-content-assets.md) and `strategy/TOOL_DECISIONS.md` before rendering. Without the slot the CSS gradient mesh carries the hero — lighter and still premium.
