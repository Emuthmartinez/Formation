...
| `check:onboarding` | `ONBOARDING.md` App Review prompt timing, cooldown, analytics, and suppressed-prompt fallback |
...
| `check:motion-contract` | Motion craft contract consistency: the spring-family bands, token table, and presets stated in `premium-mobile-craft.md`/`motion-craft-benchmarks.md` must match `tokens.json`, `PremiumCraft.swift`, and the experience-card canon; the cinematic token stays out of the in-app doctrine |
