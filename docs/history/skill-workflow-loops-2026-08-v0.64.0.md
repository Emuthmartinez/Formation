...
| 18 | Launch trace & build contracts | Crossing research → product/design/build; deciding if `engineering/TECH_SPEC.md` is needed | L18 | ✅ |
...
| 36 | Backend data contract | Before schema/auth prompts or `engineering/TECH_SPEC.md` data/API sections harden | L36 | ✅ |
...
or (c) a discrete recorded state in `state/PROJECT_STATE.yaml` (a lane set to
...
| 3 | L09 Research spec | "spec and name are evidence-backed and locked" | every claim row in the `product/SPEC.md`/`strategy/RESEARCH.md` evidence ledger carries a source cell (grep finds no empty source) **and** `lanes.research: locked` |
| 4 | L18 Launch trace | "every build decision traces to evidence and the lexicon is locked" | every row in the `state/LAUNCH_TRACE.md` decision table has a non-empty evidence cell (grep) **and** the Lexicon Lock grep returns 0 cross-surface term mismatches |
| 5 | L28 Listing packet | "complete and reconciled" | `APP_STORE_LISTING.md` has no `TODO`/empty required-field token (grep) **and** its App Privacy answers diff-match `trust/PRIVACY.md` **and** the founder-approval gate row exists |
| 6 | L41 Privacy/terms | "exist and reconcile" | `trust/PRIVACY.md`+`trust/TERMS.md` have no placeholder token (grep) **and** App Privacy/Data Safety answers diff-match the `check:apple-requirements`/`check:google-play` packets |
...
| 10 | L38 MobAI demos | "demo artifacts exist with a rerender path" | `growth/DEMO_VIDEO.md`/`CONTENT_ASSETS.md` rows have non-empty `source` + `rerender` fields (grep) — else the MobAI-access blocker is recorded in `state/PROJECT_STATE.yaml` |
...
- **Memory** always includes the relevant `state/PROJECT_STATE.yaml` lane plus the
  lane's durable artifact — grounded in SKILL.md *Operating Posture* ("Keep
  `state/PROJECT_STATE.yaml` current") and *Autopilot Run Contract*.
...
- **Action:**
  1. Read the Session Continuity source set: `AGENTS.md`, `state/PROJECT_STATE.yaml`, `state/launch-cockpit.html`, `operations/ORCHESTRATION.md`, `engineering/PRODUCTION_READINESS.md`, `operations/FAILURE_CARDS.md`, and `git status --short`. → `Start Here` (Session Continuity).
...
- **Memory:** `state/PROJECT_STATE.yaml` (phase, lanes, blockers) re-confirmed; next action left in state before any pause.
- **Stopping condition:** `check:continuity-contract` passes **and** the `next_action` field in `state/PROJECT_STATE.yaml` is non-empty; resume the in-flight lane.
...
- **Stopping condition:** `validate:launch-state` passes, `render:launch-cockpit` exits 0 and writes `state/launch-cockpit.html`, **and** `project.launch_tier` is set to `full|lite`. (Single-pass: re-runs only when a lane/provider/proof/gate field changes, never on a timer, so it cannot spin.)
...
- **Memory:** `strategy/TOOL_DECISIONS.md` + `state/PROJECT_STATE.yaml` tool-routing entries.
- **Stopping condition:** `check:paid-tool-decisions` passes **and** every lane row in `strategy/TOOL_DECISIONS.md` reads exactly one of `confirmed-paid|founder-approved-fallback|blocked-pending-access` (grep finds no unmarked row).
...
- **Memory:** `SECRETS.md` + `state/PROJECT_STATE.yaml` secrets entries.
...
- **Action:**
  ...
  2. Confirm the archetype-specific variant (content/surface/optional systems) via AskUserQuestion and record it in `state/PROJECT_STATE.yaml`. → `Start Here`.
...
- **Memory:** `state/PROJECT_STATE.yaml` archetype field + the copied starter scaffold.
- **Stopping condition:** `check:app-archetype` and `check:archetype-starter` both pass **and** the archetype field in `state/PROJECT_STATE.yaml` is set (the pack feeds the normal lanes; it does not bypass them).
...
- **Stopping condition:** `check:provider-proof` passes — each provider lane in `operations/PROVIDER_PROOF.md` carries a probe/log artifact **or** a named founder-only gate; no provider lane reads `done` without one.
...
- **Memory:** `state/LAUNCH_TRACE.md`/`CHANGE_LOG.md` + `state/PROJECT_STATE.yaml` lane evidence; `state/launch-cockpit.html` re-rendered.
- **Stopping condition:** The cascade table enumerates every surface from the matching Change Cascade Map rows (non-empty), every row is marked `updated|unaffected` (grep finds none unmarked), the Lexicon Lock grep returns 0 cross-surface mismatches, and the cascade is recorded in `state/LAUNCH_TRACE.md`/`CHANGE_LOG.md` — otherwise the `change-cascade-incomplete` failure card stays open.
...
- **Action:**
  ...
  2. Quarantine untrusted public content (reviews/social/scraped pages) and keep an evidence ledger in `strategy/RESEARCH.md`. → *Claude vs Codex Routing And Dynamic Workflows*; Phase 1.
  3. Produce the revised `product/SPEC.md` with the evidence ledger; lock the name before ASO/landing depend on it. → Phase 1; *Operating Posture* (Lock phase outputs).
- **Proof:** `strategy/RESEARCH.md` evidence ledger + `product/SPEC.md` with each claim sourced to App Store/competitor/review/XPOZ/funnel evidence.
- **Memory:** `strategy/RESEARCH.md`, `product/SPEC.md` + `state/PROJECT_STATE.yaml` research lane.
- **Stopping condition:** The `strategy/RESEARCH.md`/`product/SPEC.md` evidence ledger has ≥1 claim row and none has an empty source cell (grep finds no empty source), a name-collision result is recorded, and `lanes.research` reads `locked`.
...
- **Memory:** `LOCALIZATION_MARKET_RESEARCH.md` + `state/PROJECT_STATE.yaml` localization lane.
...
- **Memory:** `analytics/ANALYTICS.md` + `state/PROJECT_STATE.yaml` analytics lane.
- **Stopping condition:** `check:analytics-catalog` and `check:attribution` both pass — every event named in ONBOARDING/EMOTIONAL_DESIGN/VIRAL_GROWTH resolves in `analytics/ANALYTICS.md`, and the self-reported attribution fields are all present.
...
- **Memory:** `11_STAR_EXPERIENCE.md` + `state/PROJECT_STATE.yaml` 11-star lane.
...
- **Memory:** `EMOTIONAL_AUDIT.md` + `state/PROJECT_STATE.yaml` emotional_design lane evidence.
...
- **Memory:** `PAID_UA.md` + `state/PROJECT_STATE.yaml` paid_ua lane.
...
- **Memory:** `VIRAL_GROWTH.md` + `state/PROJECT_STATE.yaml` viral_growth lane.
...
- **Memory:** `growth/LAUNCH_NARRATIVE.md` + `state/PROJECT_STATE.yaml` launch_narrative lane.
...
- **Memory:** `state/LAUNCH_TRACE.md`, `engineering/TECH_SPEC.md` + `state/PROJECT_STATE.yaml` trace lane.
- **Stopping condition:** The `state/LAUNCH_TRACE.md` decision table is non-empty and every row has a non-empty evidence cell (grep) **and** the Lexicon Lock grep returns 0 cross-surface term mismatches; `engineering/TECH_SPEC.md` exists when implementation is in scope.
...
- **Memory:** `trust/SECURITY.md`, `trust/security-review.html` + `state/PROJECT_STATE.yaml` security lane.
- **Stopping condition:** `check:security` passes and `trust/security-review.html` is rendered with scan output attached to `engineering/PRODUCTION_READINESS.md` — each residual risk is marked `resolved|accepted` with a named owner, or carries a founder gate.
...
- **Memory:** `product/ONBOARDING.md` + `state/PROJECT_STATE.yaml` onboarding lane.
- **Stopping condition:** `check:onboarding` passes — the native App Review prompt sits immediately after the first value-reveal step and the paywall-timing/offer rows are present in `product/ONBOARDING.md`, before build handoff.
...
- **Memory:** `UX_PATTERNS.md` Premium Craft section + `state/PROJECT_STATE.yaml` design lane.
...
- **Memory:** `CONTENT_ASSETS.md` (and `growth/DEMO_VIDEO.md` where applicable) + `state/PROJECT_STATE.yaml` content lane.
...
- **Stopping condition:** `APP_STORE_LISTING.md` exists with every required-field section present and carries no `TODO`/empty token (grep), its App Privacy answers diff-match `trust/PRIVACY.md`, and a founder-approval gate row precedes ASC entry.
...
- **Stopping condition:** `check:apple-signing` passes — `store/APPLE_SIGNING.md` records each of account/Team ID/bundle ID/cert/profile/archive/upload state, with founder-only enrollment/credential steps flagged as named gates.
...
- **Memory:** `store/APPLE_APP_STORE_REQUIREMENTS.md` + `state/PROJECT_STATE.yaml` apple-requirements lane.
- **Stopping condition:** `check:apple-requirements` passes — the validator confirms `store/APPLE_APP_STORE_REQUIREMENTS.md` matches the manifest, required-reason APIs, and App Privacy answers, with a founder-approval gate row before upload.
...
- **Stopping condition:** `asc` commands exit 0, **or** each blocker is recorded in `state/PROJECT_STATE.yaml` as a founder-gated step with the exact next command (grep finds no bare "cannot access ASC" string).
...
- **Memory:** `SCREENSHOTS.md` + `state/PROJECT_STATE.yaml` screenshots lane.
...
- **Stopping condition:** `check:google-play` passes — `store/GOOGLE_PLAY_RELEASE.md` Data Safety diff-matches the iOS privacy labels and the closed-testing gate is on the calendar, or each blocker is recorded.
...
- **Action:**
  ...
  3. Produce `engineering/ENGINEERING_PLAN.md`, `operations/ORCHESTRATION.md`, and `engineering/PRODUCTION_READINESS.md`; serialize shared resources and run MobAI/native iOS E2E (L38/L39). → Phase 5b.
...
- **Memory:** `engineering/ENGINEERING_PLAN.md`, `operations/ORCHESTRATION.md`, `engineering/PRODUCTION_READINESS.md` + `state/PROJECT_STATE.yaml` engineering lane.
- **Stopping condition:** `check:compound-engineering` and `check:orchestration` both pass — `engineering/PRODUCTION_READINESS.md` carries an evidence artifact for each of the five stages (plan/slice/review/test/proof); the `engineering` lane stays `partial` until all five are present.
...
- **Stopping condition:** `check:backend-contract` passes — `engineering/TECH_SPEC.md` Data Contract records backend selection + reason, data model, a passing authz test (RLS/security-rule), and migrations/environments.
...
- **Stopping condition:** At least one demo artifact row exists in `growth/DEMO_VIDEO.md`/`CONTENT_ASSETS.md` with non-empty `source`/`rerender` fields (grep), **or** the MobAI-access blocker is recorded in `state/PROJECT_STATE.yaml`; no free-fallback downgrade without `check:paid-tool-decisions` approval.
...
- **Stopping condition:** `check:revenue` and `probe:revenuecat` both pass — `revenue/REVENUE_OPS.md` records a sandbox purchase granting the entitlement in app+backend, the production gate is marked founder-only, and prices diff-match `APP_STORE_LISTING.md`.
...
- **Stopping condition:** `trust/PRIVACY.md` and `trust/TERMS.md` exist, are non-empty, and carry no placeholder token (grep), and their App Privacy/Data Safety answers diff-match the `check:apple-requirements`/`check:google-play` packets; publishing stays a founder gate.
...
- **Stopping condition:** `check:email` passes — `growth/EMAIL_OPS.md` records the SPF/DKIM basis, unsubscribe handling, the design/DESIGN.md brand-token mapping, and a Resend send/webhook log proof.
...
- **Memory:** landing repo/assets + `state/PROJECT_STATE.yaml` funnel lane.
...
- **Stopping condition:** `growth/UGC_PLAYBOOK.md` contains all five named rows — fit decision, 90-day plan, budget, disclosure rules, stop/scale thresholds — each present and non-empty (grep finds none missing or empty); creator payments/contracts stay a named founder gate.
...
- **Stopping condition:** `growth/FASTLANE_OPS.md` exists with the workspace, connections, Blitz-angles, and weekly-loop sections present and a QA-pass log line recorded; social-account connection and posting/scheduling stay named founder gates.
...
- **Stopping condition:** `check:post-launch` passes — `operations/POST_LAUNCH_OPS.md` carries the weekly rhythm with a review-response SLA + crash route + cohort source, and `operations/LAUNCH_RETRO.md` exists at the +7/30/90 cadence.
...
- **Stopping condition:** `launchbench`, `check:lane-coverage`, and `test:validators` all pass — readiness is claimed only on green, with any remaining miss recorded as an open `operations/FAILURE_CARDS.md` row.
...
- **Memory:** `operations/BUSINESS_ACCESS.md`, `operations/business-access.json`.
- **Stopping condition:** `check:founder-operator` passes — `operations/BUSINESS_ACCESS.md` exists with its required sections and the rendered cockpit phase label equals the `phase` field in `state/PROJECT_STATE.yaml`. The validator fails closed on a missing artifact, so the stop cannot fire before the bootstrap runs.
...
- **Proof:** `check:agent-operations` passes; provider claims resolve to `operations/PROVIDER_PROOF.md`.
- **Memory:** `operations/AGENT_OPERATIONS.md`, `operations/agent-operations.json`, `operations/PROVIDER_PROOF.md`.
...
- **L01 vs L50** — both touch `skill-versioning.md`, but L01 is the *consumer-side* freshness gate (founder asks to update before doing launch work) and L50 is the *maintainer-side* sync after changing the skill. Distinct trigger, distinct artifact (`state/PROJECT_STATE.yaml` decision vs `skill-version.json` bump).
...
- **L51 vs L05** — both touch access and credentials, but L05 owns *secret routing* (Doppler, `SECRETS.md`, names-only `.env.example`) and L51 owns *founder access and identity* (`operations/BUSINESS_ACCESS.md`, which accounts exist and who can reach them). Separate artifacts, separate validators (`check:secrets` vs `check:founder-operator`).
- **L52 vs L07** — L07 is the *proof* loop that gates marking a provider-backed lane done (`operations/PROVIDER_PROOF.md`); L52 is the *ledger* loop that records each authenticated action as it happens (`operations/AGENT_OPERATIONS.md`). L52 feeds L07 rather than duplicating it.
...
- **Concrete anchor per stop:** 56/56. Every stop names ≥1 of — a `package.json`
  ...
  named artifact, or a discrete `state/PROJECT_STATE.yaml` lane/gate state. A grep of
...
- **Validator coverage:** each `check:*`/`probe:*` script is the terminator of the
  ...
  grep/`diff`-checkable artifact property plus a `state/PROJECT_STATE.yaml` gate, never
  on an adjective.
