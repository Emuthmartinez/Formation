...
`check:founder-copy` scans the engine's generated business surfaces; nothing scans `platform/server/domain/presentation.mjs` or the pages for banned founder vocabulary, so a violation there ships with a green audit (the 2026-08-18 review caught one by hand). `presentation.test.mjs` now scans the board tables for the worst offenders, but the durable fix is a platform-side vocabulary gate sharing the engine's banned-word source.
