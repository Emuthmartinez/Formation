...
| validation/repository/check-adapter-contract.ts | keep | no v1 precedent — new post-U8 file (2026-08-19), not a port; adapter contract A's golden-sample gate (layering plan R1): positive goldens validate, negative controls must fail, and the goldens' contractVersion must equal the code's — wired in both manifests since the schemas and fixtures ship in the runtime |
