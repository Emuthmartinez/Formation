...
| `check:launch-trace` | Research reaches product, brand, design, store copy, revenue, privacy, and verification through `state/LAUNCH_TRACE.md` |
...
| `check:product-spec` | `product/SPEC.md` scope locks, acceptance criteria, and — at a done product lane — the Differentiation And Moat contract: two research-grounded incumbent rows, an affirmed moat class with its build plan (or the named-dated-day-30 V1 exception), and a substantive one-week-copy answer |
...
| `check:privacy-terms` | `trust/PRIVACY.md` and `trust/TERMS.md` coverage against what the app actually collects |
